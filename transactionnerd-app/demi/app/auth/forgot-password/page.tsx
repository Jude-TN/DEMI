        ...
        <div style={{ padding: "24px 22px 20px" }}>
          {sent ? (
            <>
              ...
              <p style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.6, marginBottom: 16 }}>Reset link sent to <strong style={{ color: "var(--teal)" }}>{email}</strong>.</p>
            ...
            <>
              ...
              <p style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.6, marginBottom: 20 }}>Enter your email and we'll send a reset link.</p>
              <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)" }}>Email</label>
                  <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" />
                </div>
                <button type="submit" disabled={loading} style={{ background: "var(--teal)", color: "#0a1412", border: "none", borderRadius: 6, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", opacity: loading ? .7 : 1 }}>
